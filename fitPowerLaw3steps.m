...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    try % try an OLS regression first
        ...
        [ intSlo, stat] = robustfit(Xi,Yi,'ols'); % figure,plot(Xi ,Yi ); hold on; plot( Xi(~boolYdev),Yi(~boolYdev),'k')
    end
...
YYpred= 10.^Ypred;  
resPow= YYi- YYpred' ; %%%%% DEFAULT: delta in PSD units %%%%%%%%%%%%%%%%%%%%%
% resPow= ( Yi- Ypred') ;  %%%% NEW: first delta in log10-units,%% then it's wrong to get exp 10.^

% figure; plot(XXi, resPow)
...
YYpredPos= (YYi(idxPos))- YYpred(idxPos)'; % corrected, used to be Yi till 16/7/19
Pows.meanResPos= mean( 10.^ YYpredPos); %YYpred(idxPos)'
Pows.cenFrObs= sum(XXi.*YYi)./sum(YYi);
Pows.cenFrRes= sum(XXi.*resPow)./sum(resPow);

tmp=  YY ./sum(YY);
Pows.entropy= -sum( tmp .* log2(tmp)) ;%./ log2(length(tmp));
